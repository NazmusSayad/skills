...
Prioritize simplicity, clarity, and readability over reusability or abstraction. Do not introduce variables, functions, helpers, types, or other abstractions unless the logic becomes very large and extremely complex or repeated many times.
...
Do not write tests (eg, unit tests, regression tests) or run validation commands or checks unless requested or absolutely necessary. When possible, run multiple validation commands together.
